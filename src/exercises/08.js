...
class Toggle extends React.Component {
    ...
    internalSetState = (changes, callback) => (
        this.setState(state => {
            ...
            const reducedChanges = this.props.stateReducer(state, changesObj) || {}

            // return null if there are no changes to be made
            // (to avoid an unnecessary rerender)
            return Object.keys(reducedChanges).length && reducedChanges.constructor === Object
                ? reducedChanges
                : null
    ...
    getTogglerProps = ({onClick, ...props} = {}) => ({
        onClick: callAll(onClick, this.toggle), // allow child to call something else besides toggle function
        'aria-pressed': this.state.on, // internal state of component
        ...props, // add additional props from child
    })
    ...
    render() {
        return this.props.children(this.getStateAndHelpers()) // give children full context of parent
    }
}
